...
const Form = () => {
 ...
 useEffect(() => {
  ...
  const generateButton = <button disabled={loading} onClick={generate} >{loading ? <div className="lds-ring">
    ...
    <textarea placeholder="Describe your business industry/niche (required):" required value={formData.description} onChange={e => setFormData(prev => {
      setValidation(null)
      return ({...prev, description: e.target.value})
    })} name="description" id="description" type="text"/><br/>
    {validatation?.description && <div className="validation"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{validatation?.description}</div>}
    <input placeholder="Keywords (optional): keywords you want to add to your business name" value={formData.keyword} name="keyword" id="keyword" type="text" onChange={e => setFormData(prev => ({...prev, keyword: e.target.value}))}/><br/><br/>
